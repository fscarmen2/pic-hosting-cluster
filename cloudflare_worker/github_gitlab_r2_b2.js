...
async function checkR2Storage(r2Config) {
  try {
    ...
    if (response.ok) {
      ...
      while ((sizeMatch = sizeRegex.exec(data)) !== null) {
        sizes.push(parseInt(sizeMatch[1]));
      }

      // 遍历所有文件
      keys.forEach((key, index) => {
        if (key.startsWith(`${DIR}/`) && !key.endsWith('/')) {
          fileCount++;
...
async function deleteGitHubFile(repo, filePath, pat) {
  ...
  try {
    ...
    return `删除失败：(${getResponse.statusText})`;
  } catch (error) {
...
async function deleteR2File(r2Config, filePath) {
  ...
  try {
    // 1. 首先检查文件是否存在
    const headRequest = await getSignedUrl(r2Config, 'HEAD', `${r2Config.bucket}/${fullPath}`);
    ...
    // 2. 文件存在，执行删除
    const signedRequest = await getSignedUrl(r2Config, 'DELETE', `${r2Config.bucket}/${fullPath}`);
    ...
    return `删除失败：(${response.statusText})`;
  } catch (error) {
...
async function deleteB2File(b2Config, filePath) {
  ...
  try {
    // 1. 首先列出文件的所有版本
    const listVersionsPath = `${b2Config.bucket}/${fullPath}`;
    const signedListRequest = await getSignedUrl(b2Config, 'GET', listVersionsPath, {
      'versions': ''
    });

    const listResponse = await fetch(signedListRequest.url, {
      headers: {
        ...signedListRequest.headers,
        'Host': b2Config.endPoint
      }
    });
    ...
    const versionIds = [];
    const versionRegex = /<VersionId>([^<]+)<\/VersionId>/g;
    let match;
    while ((match = versionRegex.exec(listData))!== null) {
    ...
    for (const versionId of versionIds) {
      const deleteVersionPath = `${b2Config.bucket}/${fullPath}?versionId=${versionId}`;
      const signedDeleteRequest = await getSignedUrl(b2Config, 'DELETE', deleteVersionPath);

      await fetch(signedDeleteRequest.url, {
        method: 'DELETE',
        headers: {
          ...signedDeleteRequest.headers,
          'Host': b2Config.endPoint
        }
      });
    }
